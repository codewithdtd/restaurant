    ...
    <div class="user">
        <div class="user__form" :class="{'user__form--active': user_form }">
            <form action="" @submit.prevent="handleSubmit">
                ...
                <div class="user__form__item">
                    <p>Số điện thoại:</p>
                    <input required type="text" v-model="user.phone">
                </div>
                <div class="user__form__item">
                    <p>Địa chỉ:</p>
                    <textarea name="" id="" cols="30" rows="3">{{ user.address }}</textarea>
                </div>
                <button>Xác nhận</button>
            </form>
        </div>
        <TableCustomer ref="tableCustomer" :nameTable="this.$route.name" @edit="handleEdit"></TableCustomer>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        async handleUser() {
            ...
            await this.$refs.tableCustomer.getAllUser();
        },
        handleEdit(order) {
            this.user = order;
        ...
        async handleSubmit() {              
            if(this.user._id != null) {
                if(await userService.updateUser(this.user)) {
                    this.user_form = !this.user_form;
                }
            }
